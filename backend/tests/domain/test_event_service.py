...
async def test_get_event_uses_live_contract_supply_when_available():
    ...
    service = EventService(
        FakeEventRepository([existing]),
        FakeContractDeployer(minted_counts={contract_address: 1}),
    )

    event = await service.get_event("aurora-city-live")

    assert event.categories[0].minted_count == 1
